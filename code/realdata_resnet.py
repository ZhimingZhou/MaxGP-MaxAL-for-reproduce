...
tf.app.flags.DEFINE_string("sResultTagA", "test", "your tag for each test case")
tf.app.flags.DEFINE_string("sResultTagB", "", "your tag for each test case")

##################################################### Objectives ##############################################################################################

tf.app.flags.DEFINE_string("sGAN_type", 'x', "")
...
tf.app.flags.DEFINE_string("sGP_type", 'gp', 'cp, gp, lp, al')
...
tf.app.flags.DEFINE_integer("iMaxIter", 200000, "")
...
tf.app.flags.DEFINE_string("oOptG", 'Adam', "")
tf.app.flags.DEFINE_string("oOptD", 'Adam', "")
...
lr_dict = {0.0002: '2e-4', 0.0001: '1e-4', 0.00005: '5e-5'}
weight_dict = {1e-5: '1e-5', 1e-4: '1e-4', 1e-3: '1e-3', 0.01: '0.01', 0.1: '0.1', 1.0: '1.0', 10.0: '10.0'}
...
sResultTag += '_' + cfg.sGAN_type + ('_%.2f' % cfg.alpha if cfg.sGAN_type in ['lsgan', 'hinge'] else '')
sResultTag += '_SND' if cfg.bUseSND else ''
sResultTag += ('_lip_' + ('max' if cfg.bMaxGrad else '') + cfg.sGP_type + '%.e' % cfg.fWeightLip) if cfg.bLipReg else ''
sResultTag += ('_fBF%.2f' % cfg.fBufferBatch) if (cfg.fBufferBatch != 0 and cfg.bLipReg) else ''
sResultTag += ('_fAL%.e' % cfg.fLrAL) if ('al' in cfg.sGP_type and cfg.fLrAL != 0 and cfg.bLipReg) else ''
...
if 'Linear' == cfg.oDecayG:
    gen_lr = tf.constant(cfg.fLrIniG) * tf.maximum(0., 1. - (tf.cast(global_step, tf.float32) / cfg.iMaxIter))
elif 'Exp' == cfg.oDecayG:
    gen_lr = tf.train.exponential_decay(cfg.fLrIniG, global_step, cfg.iLrStep, cfg.fLrDecay, cfg.bLrStair)
else:
    gen_lr = tf.constant(cfg.fLrIniG)

if 'AdaShift' == cfg.oOptG:
    logger.log('gen_optimizer: AdaShift')
    gen_optimizer = AdaShift(learning_rate=gen_lr, beta1=cfg.fBeta1, beta2=cfg.fBeta2, epsilon=cfg.fEpsilon)
else:
...
if 'Linear' == cfg.oDecayD:
    dis_lr = tf.constant(cfg.fLrIniD) * tf.maximum(0., 1. - (tf.cast(global_step, tf.float32) / cfg.iMaxIter))
elif 'Exp' == cfg.oDecayD:
    dis_lr = tf.train.exponential_decay(cfg.fLrIniD, global_step, cfg.iLrStep, cfg.fLrDecay, cfg.bLrStair)
else:
...
while iter < cfg.iMaxIter:
    ...
    for i in range(cfg.iTrainD):
        if cfg.bLipReg and cfg.sGP_type == 'al':
    ...
    if (iter % (cfg.iMaxIter // 40) == 0 or iter == cfg.iMaxIter) and (cfg.sDataSet == 'cifar10' or cfg.sDataSet == 'flowers' or cfg.sDataSet == 'tiny'):
