...
tf.app.flags.DEFINE_string("sResultTagA", "case4", "your tag for each test case")
tf.app.flags.DEFINE_string("sResultTagB", "n=2", "your tag for each test case")
...
tf.app.flags.DEFINE_float("fLrIni", 0.0002, "")
tf.app.flags.DEFINE_string("oOpt", 'Adam', "")
tf.app.flags.DEFINE_string("oDecay", 'linear', "linear, exp, none")
...
tf.app.flags.DEFINE_integer("iDimsPerLayer", 256, "")
...
def plot2(names, x_map_size, y_map_size, x_value_range, y_value_range, mode='fake', contour=False):
    ...
    plt.scatter(_real_datas[:, 0], _real_datas[:, 1], marker='+', s=50, label='real samples', color='navy') #purple')#indigo')#navy')balck
    plt.scatter(_fake_datas[:, 0], _fake_datas[:, 1], marker='*', s=50, label='fake samples', color='ivory') #ivory') #'silver')white
...
def plot(names, x_map_size, y_map_size, x_value_range, y_value_range, mode='fake', contour=False):
    ...
    if cfg.sGAN_type == 'lsgan' and not cfg.bLipReg:
        cmin = -2.0
        cmax = +2.0
    else:
