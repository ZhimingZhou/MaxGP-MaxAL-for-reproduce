...
def deconv2d(input, output_dim, ksize=3, stride=1, padding='SAME', name='deconv2d',
    ...
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        if enable_wn:
            w = tf.get_variable('w', [ksize, ksize, output_dim, input_shape[c_axis]], initializer=initializer(init_distribution_type, [0, 1, 3], init_weight_stddev))
...
def conv2d(input, output_dim, ksize=3, stride=1, padding='SAME', name='conv2d',
    ...
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        if enable_wn:
            w = tf.get_variable('w', [ksize, ksize, input_shape[c_axis], output_dim], initializer=initializer(init_distribution_type, init_weight_stddev))
...
def linear(input, output_dim, name='linear',
    ...
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        if enable_wn:
            w = tf.get_variable('w', [input_shape[1], output_dim], initializer=initializer(__init_distribution_type__, [0], __init_weight_stddev__))
